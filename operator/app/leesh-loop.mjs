...
async function start(config) {
  return withLock(config, async () => {
    ...
    try {
      ...
      const env = { ...process.env, NOTION_TOKEN: notionToken, LEESH_LOOP_NOTION_DATABASE_URL: config.notion_database_url, LEESH_LOOP_WORKSPACE_FILES: JSON.stringify(config.workspace_files), SYMPHONY_WORKSPACE_ROOT: config.symphony_workspace_root, SYMPHONY_GITHUB_REPOSITORY_URL: config.github_repository_url, SYMPHONY_GITHUB_BASE_BRANCH: config.github_base_branch, SYMPHONY_EFFECTIVE_CONFIGURED_BASE_BRANCH: config.github_base_branch, SYMPHONY_SELF_VERIFICATION_RUN_ID: config.self_verification_run_id || process.env.SYMPHONY_SELF_VERIFICATION_RUN_ID || '', SYMPHONY_LIFECYCLE_EVIDENCE_PATH: config.lifecycle_evidence_path || process.env.SYMPHONY_LIFECYCLE_EVIDENCE_PATH || '', SYMPHONY_DISPATCH_BARRIER: 'closed', SYMPHONY_RUNTIME_ID: runtimeId, SYMPHONY_DISPATCH_AUTHORIZATION_FILE: p.authorization, SYMPHONY_DISPATCH_ACK_FILE: p.acknowledgement, SYMPHONY_OWNERSHIP_FILE: p.ownership, SYMPHONY_OPERATOR_STARTUP_STATUS_FILE: p.startup_status, SYMPHONY_DISPATCH_COORDINATION_ROOT: join(stateRoot(config), 'dispatch-coordination') };
