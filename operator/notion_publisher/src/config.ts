...
import { DEFAULT_POLICY, PLAN_PROPERTY, type Policy, PublicationError } from "./core.js";

const keys = new Set(["priority","labels","property_names","state_seeds"]);
...
export async function loadConfig(file:string, base?:string, policy=DEFAULT_POLICY):Promise<{config:{};policy:Policy}> {
 ...
 if("state_seeds" in r && (!Array.isArray(r.state_seeds) || !r.state_seeds.every(x=>typeof x==="string" && x.trim()))) throw new PublicationError("state_seeds must be a list of non-empty strings");
 ...
 const o=overrides as Record<string,string>; const p={...policy,defaultPriority:("priority" in r?r.priority:policy.defaultPriority) as number|null,defaultLabels:[...new Set(((r.labels as string[]|undefined)??policy.defaultLabels).map(x=>x.trim()).filter(Boolean))],stateSeeds:[...new Set(((r.state_seeds as string[]|undefined)??policy.stateSeeds).map(x=>x.trim()).filter(Boolean))],identifier:o.identifier??policy.identifier,title:o.title??policy.title,state:o.state??policy.state,priority:o.priority??policy.priority,labels:o.labels??policy.labels,blockedBy:o.blocked_by??policy.blockedBy};
 const names=[p.identifier,p.title,p.state,p.priority,p.labels,p.blockedBy].map(x=>x.trim()); if(names.some(x=>!x)) throw new PublicationError("property names must be non-empty"); if(new Set(names).size!==names.length) throw new PublicationError("property_names must resolve to unique property names"); if(names.includes(PLAN_PROPERTY)) throw new PublicationError(`property_names cannot redefine the reserved ${PLAN_PROPERTY} relation`);
