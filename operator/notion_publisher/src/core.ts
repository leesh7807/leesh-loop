...
export type Policy = { identifier:string; title:string; state:string; priority:string; labels:string; blockedBy:string; defaultPriority:number|null; defaultLabels:string[]; stateSeeds:string[] };
...
export function buildPlanBlocks(plan:string):Record<string,unknown>[] { return chunkText(normalizePlanText(plan)).map(part=>({object:"block",type:"paragraph",paragraph:{rich_text:[text(part)]}})); }
