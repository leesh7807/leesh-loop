...
export async function submit(store: NotionStore, databaseId: string, browser: BrowserTransport, prompt: string, options: SubmitOptions = {}): Promise<string> {
  ...
  try { await attempt(); const started = Date.now(); let inspection = false;
    while (true) { const current = await store.readInvocation(invocation.pageId, id); if (current.state !== 'pending') { log('acknowledged', id); if (current.state === 'completed') { log('terminal_completed', id); return markdownResult(store, invocation.pageId); } if (!current.error.trim()) fail('INVALID_INVOCATION_STATE', `Invocation ${id} failed without Error.`); log('terminal_failed', id); fail('INVOCATION_FAILED', current.error); }
      const elapsed = Date.now() - started;
      if (!inspection && elapsed >= ackMs) { inspection = true; log('submission_inspection_started', id); const result = await browser.inspectSubmission(id); if (result === 'not_submitted' && attempts < 2) { log('submission_retry_attempted', id); await attempt(); continue; } if (result === 'uncertain') fail('SUBMISSION_UNCERTAIN', `Submission status for ${id} is uncertain; it was not retried.`); if (result === 'not_submitted') fail('ACKNOWLEDGMENT_TIMEOUT', `Second submission was not acknowledged for ${id}.`); }
