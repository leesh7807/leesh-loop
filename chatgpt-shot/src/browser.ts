...
async function connectRuntime(root: string): Promise<Browser> {
  const remembered = readRuntime(root); const active = remembered && await endpoint(remembered);
...
export class ChatGPTBrowser implements BrowserTransport {
  ...
  private async start(): Promise<Page> { if (this.page) return this.page; try { this.browser = await connectRuntime(this.root); this.context = this.browser.contexts()[0] ?? await this.browser.newContext(); this.page = this.context.pages()[0] ?? await this.context.newPage(); await this.page.goto('https://chatgpt.com/', { waitUntil: 'domcontentloaded', timeout: 30_000 }); return this.page; } catch (e) { return fail('BROWSER_UNAVAILABLE', 'Could not attach to the managed local ChatGPT browser runtime.', e); } }
  ...
  async close() { await this.browser?.close(); this.page = undefined; this.context = undefined; this.browser = undefined; }
}
