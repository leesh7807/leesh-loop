...
class SubmissionLock {
  ...
  constructor(root: string) { this.path = join(root, '.chatgpt-shot-submit.lock'); this.owner = join(this.path, 'owner.json'); }
...
export class ChatGPTBrowser implements BrowserTransport {
  ...
  async runSubmission<T>(operation: () => Promise<T>): Promise<T> { return new SubmissionLock(this.root).run(operation); }
