...
export async function ensureBroker(root: string) {
  ...
  try { await request(root, 'ensure'); return; } catch (error: any) {
    if (!absent(error)) throw error;
    if (error.code === 'ECONNREFUSED') try { unlinkSync(brokerSocket(root)); } catch {}
  }
  ...
  for (let attempt = 0; attempt < 50; attempt++) { try { await request(root, 'ensure'); return; } catch (error: any) { if (!absent(error)) throw error; await wait(100); } }
...
export class ChatGPTBrowser implements BrowserTransport {
  ...
  async submitPrompt() {
    ...
    try { await request(this.root, 'submit', this.sessionId); }
