...
async function main(args: string[]) {
  ...
  if (command === 'doctor') { const store = await initialized(config); const browser = new ChatGPTBrowser(config.root); await browser.withBrowser(async () => { await browser.ensureAvailable(); await browser.ensureAuthenticated(); await browser.openFreshContext(); }); out('OK: repository configuration, Invocation database, browser profile, authenticated ChatGPT session, and composer are available.'); return; }
