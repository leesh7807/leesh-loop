...
import { NotionStore, pageIdFromUrl } from './notion.js';
import { ChatGPTBrowser } from './browser.js';
import { submit } from './service.js';

const out = (value: string) => process.stdout.write(`${value}\n`);
async function initialized(config: ReturnType<typeof loadConfig>): Promise<NotionStore> { const databaseId = config.databaseId; if (!databaseId) return fail('CONFIG_INVALID', 'NOTION_INVOCATION_DATABASE_ID is required; run `chatgpt-shot init --notion-page <url>`.'); const store = new NotionStore(config.notionToken); store.validateSchema(await store.database(databaseId)); return store; }
async function main(args: string[]) {
  const [command, ...rest] = args; const config = loadConfig();
  if (command === 'init') { const pos = rest.indexOf('--notion-page'); const url = pos >= 0 ? rest[pos + 1] : undefined; if (!url) return fail('CONFIG_INVALID', 'Usage: chatgpt-shot init --notion-page <url>'); const store = new NotionStore(config.notionToken); const parent = pageIdFromUrl(url); await store.pageAccessible(parent); const configured = config.databaseId; if (configured) { store.validateSchema(await store.database(configured)); out(`already initialized: ${configured}`); return; } const created = await store.createDatabase(parent); store.validateSchema(await store.database(created.id)); persistDatabaseId(config, created.id); out(`initialized: ${created.id}`); return; }
  if (command === 'login') { const browser = new ChatGPTBrowser(config.root, true); await browser.ensureAvailable(); out('ChatGPT browser opened. Authenticate manually, then close this command with Ctrl+C.'); await new Promise<void>(resolve => process.once('SIGINT', resolve)); await browser.close(); return; }
