...
class Browser {
  attempts = 0; opens = 0; inspections = 0; closes = 0; inspected: 'submitted'|'not_submitted'|'uncertain' = 'submitted'; authenticated = true; openError?: Error; submitError?: Error;
  ...
  async openFreshContext() { this.opens++; if (this.openError) throw this.openError; } async fillPrompt() {} async submitPrompt() { this.attempts++; if (this.submitError) throw this.submitError; } async inspectSubmission() { this.inspections++; return this.inspected; } async close() { this.closes++; }
}
