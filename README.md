...
The Publisher does not plan or execute work, synchronize later repository Plan corrections back to Notion, or decide workflow behavior. The publication/execution contract requires it to validate an execution-eligible Plan's declared repository-relative Plan reference and make the normalized Description begin with `Repository Plan Reference: <repository-relative path>`. The adapter carries that existing Description unchanged into Symphony's `issue.description`; it neither parses nor interprets the Plan, and no Symphony issue-domain field is added. This binds each task to one writable repository Plan without making the published snapshot mutable. The Plan stays active through review and rework, then moves to completed only with terminal task completion. Until this carrier is implemented, a publication can exist but is not eligible for autonomous execution under this workflow.
