...
This directory is the Elixir/OTP implementation of Symphony, based on [`SPEC.md`](SPEC.md).
...
3. Create a `WORKFLOW.md` in the repository that will call Symphony. Start with the minimal
   example below and customize it for your project.
4. Customize your `WORKFLOW.md` file for your project.
...
`--i-understand-that-this-will-be-running-without-the-usual-guardrails` is required. Symphony
exits with code 1 when it is omitted.

## Burrito local builds
...
This repository does not publish Burrito artifacts or provide a release workflow. Build the
configured release locally when needed:

```bash
mise exec -- mix release symphony
```
...
- `WORKFLOW.md`: caller repository's runtime configuration and prompt input; Symphony accepts an
  explicit path or discovers this caller-owned file in its current working directory.
