...
export type PublishInput = { plan: string; databaseUrl: string; client: NotionClient; config: PublisherConfig };
export type PublishResult = { identifier: string; page_id: string; url?: string };

export async function publish({ plan, databaseUrl, client, config }: PublishInput): Promise<PublishResult> {
