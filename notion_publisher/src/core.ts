...
export function notionId(value:string):string { let raw=value; try { const u=new URL(value); if (!["http:","https:"].includes(u.protocol) || !u.hostname) throw new Error(); raw=u.pathname.split("/").pop()??""; } catch { raw=value; } const match=raw.match(/([\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}|[\da-f]{32})$/i); if(!match) throw new PublicationError("invalid target URL: expected a Notion page URL ending in a 32-character page id"); const compact=match[1].replace(/-/g,""); return `${compact.slice(0,8)}-${compact.slice(8,12)}-${compact.slice(12,16)}-${compact.slice(16,20)}-${compact.slice(20)}`; }
export async function loadConfig(file:string, base?:string, policy=DEFAULT_POLICY):Promise<{config:{parentId:string;parentUrl:string;planSource?:string};policy:Policy}> {
 ...
 if("plan_source" in r && typeof r.plan_source!=="string") throw new PublicationError("plan_source must be a string URL or path");
...
export const chunkText=(text:string,limit=1900)=>text.length?Array.from({length:Math.ceil(text.length/limit)},(_,i)=>text.slice(i*limit,(i+1)*limit)):[""];
const text=(content:string)=>({type:"text",text:{content}});
export function buildTaskProperties(p:Policy,id:string,title:string,source?:string):Record<string,unknown>{return {[p.identifier]:{rich_text:[text(id)]},[p.title]:{title:[text(title)]},[p.state]:{select:{name:p.defaultState}},[p.priority]:{number:p.defaultPriority},[p.labels]:{multi_select:p.defaultLabels.map(name=>({name}))},[p.description]:{rich_text:[text("Completed plan artifact; see Plan section.")]},[p.source]:{url:source?.startsWith("http")?source:null}};}
