...
export function buildTaskProperties(p:Policy,id:string,title:string,source?:string,state=p.defaultState):Record<string,unknown>{return {[p.identifier]:{rich_text:[text(id)]},[p.title]:{title:[text(title)]},[p.state]:{select:{name:state}},[p.priority]:{number:p.defaultPriority},[p.labels]:{multi_select:p.defaultLabels.map(name=>({name}))},[p.description]:{rich_text:[text("Accepted Plan snapshot; see Plan section.")]},[p.source]:{url:source??null}};}
