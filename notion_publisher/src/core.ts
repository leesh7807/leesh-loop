...
export function notionId(value:string):string { let raw:string; try { const u=new URL(value); if (!["http:","https:"].includes(u.protocol) || !u.hostname) throw new Error(); raw=u.pathname.split("/").pop()??""; } catch { throw new PublicationError("invalid target URL: expected an HTTP(S) Notion page URL"); } const match=raw.match(/([\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}|[\da-f]{32})$/i); if(!match) throw new PublicationError("invalid target URL: expected a Notion page URL ending in a 32-character page id"); const compact=match[1].replace(/-/g,""); return `${compact.slice(0,8)}-${compact.slice(8,12)}-${compact.slice(12,16)}-${compact.slice(16,20)}-${compact.slice(20)}`; }
