...
const keys = new Set(["state","priority","labels","plan_source"]);
...
export async function loadConfig(file:string, base?:string, policy=DEFAULT_POLICY):Promise<{config:{planSource?:string};policy:Policy}> {
 ...
 const p={...policy,defaultState:("state" in r?r.state:policy.defaultState) as string,defaultPriority:("priority" in r?r.priority:policy.defaultPriority) as number|null,defaultLabels:[...new Set(((r.labels as string[]|undefined)??policy.defaultLabels).map(x=>x.trim()).filter(Boolean))]};
