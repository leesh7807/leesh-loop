...
This is **initial task binding**, not normal continuation. The Symphony `attempt` template variable is the binding signal: absent/null means the first attempt; an integer means retry or continuation. Only when `attempt` is absent/null, fetch the stated base commit from the configured repository remote, verify it, and check out that exact commit in the new task workspace. Use the required agent-side task surface to read the immutable accepted Plan snapshot, then materialize that snapshot at the referenced active path and verify the resulting bytes. Record an idempotent Workpad `binding-established` checkpoint containing the base commit, snapshot identity, and materialized path. Only then open that file; do not scan `docs/plans/active/`, infer from a title, or choose a similarly named Plan. Treat it as the durable baseline for objective, boundaries, decisions, assumptions, constraints, and verification design. It is not an immutable prediction of implementation steps.

When `attempt` is an integer, reuse the existing workspace, task branch, Workpad, and repository Plan exactly as the previous attempt left them. Do not check out the base commit again and do not rematerialize the accepted snapshot over durable Plan corrections. If the workspace or `binding-established` checkpoint is missing or inconsistent, do not silently initialize again: record a recovery blocker and require operator-directed recovery.
