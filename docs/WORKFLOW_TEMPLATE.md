...
- If the checkpoint is absent and the workspace is still safely unbound—at the stated base commit, without task changes, and without a conflicting Plan at the materialization path—perform initial binding idempotently: fetch and check out the base commit, read the accepted snapshot, materialize and verify it, then append the checkpoint.
- If the checkpoint is absent but the exact accepted snapshot is already materialized at the path and the workspace has no other task changes, append the missing checkpoint without rematerializing the file.
