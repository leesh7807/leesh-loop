...
* a distinct Workpad surface reserved for later execution and handoff.
...
A task-owned execution surface reserved for later implementation and coordination.
...
Later execution tooling may use the Workpad to record information needed to continue or review execution, such as:
...
For this publisher, the Workpad requirement is limited to establishing that separate surface.

The publisher:

* creates the Workpad as a distinct section;
* initializes it with no execution content;
* does not infer or generate Workpad contents from the Plan;
* does not read, interpret, replace, or mutate later Workpad content.

Workpad read and mutation behavior belongs to the later coordination or execution layer.
...
* task page structure created during publication;
* default values;
* bootstrap behavior;
* validation rules;
...
The policy defines the existence and location of the Workpad surface, but it does not define later Workpad mutation semantics.
...
* the canonical task-page structure.
...
The publisher also creates a distinct empty Workpad section.
...
* implement later Workpad read or mutation behavior.
...
Do not require the adapter to interpret plan prose in order to reconstruct missing structured semantics.
...
The publisher is responsible for creating that surface, not for implementing the later adapter behavior that consumes it.
...
The later coordination adapter maps each related page to a blocker reference containing at least:
...
The publisher must create Plan and Workpad as distinct page surfaces.

This distinction must be deterministic enough that later coordination tooling can build on the resulting structure without requiring the publisher to implement that tooling now.

The publisher does not define how arbitrary future Workpad content is structured, read, replaced, or updated.
...
The later coordination adapter exposes this section as:
...
Its purpose in this work is to reserve a separate execution surface alongside the immutable Plan.

The publisher owns:

* creation of the Workpad section;
* its structural separation from the Plan;
* its initial empty state.

The publisher does not own:

* Workpad read semantics;
* Workpad update semantics;
* replacement behavior;
* Workpad internal headings or structure;
* failed-update recovery;
* concurrent Workpad mutation;
* later agent-facing Workpad APIs.

Those concerns belong to the coordination or execution layer that later consumes the published task.

The existence of Workpad in the task structure must not turn the publisher into a general Workpad storage API.

### Canonical Normalized Mapping

A future coordination adapter may normalize a Notion task as:
...
This mapping describes the intended later coordination model.

The publisher is responsible for creating the fields and page surfaces from which that model can later be constructed. It does not implement the complete normalization or Workpad access path in this work.

No canonical structured field may require interpretation of arbitrary plan prose.
...
It belongs in the page body as a dedicated execution surface.
...
Each published task page is initialized with:
...
The exact API operation order may depend on Notion IDs returned during bootstrap, but the resulting schema and initial page structure are fixed.
...
Validation or interpretation of later Workpad contents is outside publisher scope.
...
A failed publication must not leave a publisher-created partial task in a state that prevents the same plan from being safely published again.

If creation of a task begins successfully but later publication of required Plan content fails, clean up the incomplete publication or otherwise ensure that it cannot block a retry through duplicate detection.

This requirement applies to the publication transaction itself.

It does not extend to later Workpad mutations, which are outside publisher scope.
...
The Workpad is intentionally reserved for later mutable execution coordination, but that does not imply synchronization back into the plan document.
...
* the local coordination-specific empty Workpad surface reserved for later use.
...
Reading and updating Workpad is likewise outside publisher scope and belongs to later coordination/execution integration.
...
This verification does not require implementing Workpad access.
...
* the default policy defines the canonical initial Plan/Workpad page structure;
...
The policy need not implement later Workpad read or update behavior.
...
7. the published task contains both a populated Plan section and a distinct empty Workpad section.
...
The publisher does not need to inspect, parse, or validate arbitrary Workpad content on previously published task pages.
...
* a distinct Workpad section exists;
* the Workpad starts empty;
...
### Plan / Workpad Structural Separation

Demonstrate that publication produces two distinct page surfaces:
...
Verification only needs to establish that:

* the Plan content is preserved;
* Workpad exists separately from Plan;
* Workpad starts without execution content;
* publication does not mix Workpad contents into Plan.

Do not implement general Workpad read/update behavior merely to prove this separation.
...
Using a deterministic automated test or controlled failure, demonstrate that failure after the publisher has begun creating a task does not leave an incomplete task that prevents the same plan from being published again.

This verification applies to creation of the required published task and Plan content.

Do not extend this requirement to later Workpad mutations.
...
* canonical Plan/Workpad page construction;
* expected error mapping;
* provider request batching required to publish a complete Plan;
* cleanup or rollback required when a partial publication would otherwise block safe retry.

Use fake or injected Notion clients where appropriate so ordinary unit tests do not depend on external network access.

The following are not requirements of this plan:

* general Workpad reads;
* Workpad replacement;
* Workpad update batching;
* Workpad mutation rollback;
* Workpad internal-heading parsing;
* Workpad concurrency behavior.
...
No later Workpad mutation is required for this verification.
...
* verify Plan/Workpad structural separation and initial empty Workpad state;
...
Verifies deterministic publisher behavior, policy resolution, configuration handling, path handling, request construction, schema validation, initial page construction, duplicate handling, publication batching, partial-publication cleanup, and failure mapping without requiring live Notion access.

### TypeScript Type Checker

Verifies type consistency across configuration, policy, canonical task data, and Notion publication boundaries.
...
Provides a controlled real target for bootstrap, schema inspection, successful publication, initial Plan/Workpad inspection, duplicate protection, and permission verification.
