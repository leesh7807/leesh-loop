...
- Prefer asserting against a captured scheduling timestamp. Because the exact internal clock read
  is not exposed and no production defect was found, use the timestamp immediately before the
  scheduling trigger as a safe lower-bound reference and retain only the existing small upper
  scheduling-overhead slack. Use synchronous state observation instead of an arbitrary observation
  sleep.
...
- The corrected assertion uses the scheduling-associated timestamp (or a minimal justified
  tolerance) and passes through the normal orchestrator retry path.
