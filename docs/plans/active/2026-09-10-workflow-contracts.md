...
- Initial binding writes a Workpad checkpoint. That durable checkpoint and materialized workspace state, rather than Symphony retry count, determine whether a worker resumes or safely completes idempotent initial binding; only unreconcilable state is an operator recovery blocker.
...
- Symphony `attempt` is retry/continuation metadata only. The root workflow renders it without treating it as binding authority; human-judgment trigger semantics remain solely in the template.
